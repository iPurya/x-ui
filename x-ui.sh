...
install_bbr() {
    ...
    bash <(curl -L -s https://raw.githubusercontent.com/ipurya/x-ui/master/bbr.sh)
    echo ""
}

update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/ipurya/x-ui/raw/master/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
